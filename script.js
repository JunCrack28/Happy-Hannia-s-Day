...
// Miércoles 12 de agosto de 2026, 00:00 en Panamá (UTC−5).
const UNLOCK_TIME=Date.parse('2026-08-12T00:00:00-05:00');
function updateCountdown(){const remaining=Math.max(0,UNLOCK_TIME-Date.now());const seconds=Math.floor(remaining/1000);$('cdHours').textContent=String(Math.floor(seconds/3600)).padStart(2,'0');$('cdMinutes').textContent=String(Math.floor(seconds%3600/60)).padStart(2,'0');$('cdSeconds').textContent=String(seconds%60).padStart(2,'0');if(!remaining){$('countdownScreen').classList.add('unlocked');clearInterval(countdownTimer);}}
updateCountdown();const countdownTimer=setInterval(updateCountdown,1000);
...
let cutX = null, cutY = null, cutMoved = false;
function beginCut(event) { cutX=event.clientX; cutY=event.clientY; cutMoved=false; guide.style.transform='scaleX(0)'; if(booster.setPointerCapture) booster.setPointerCapture(event.pointerId); }
function continueCut(event) { if (cutX === null || cutting) return; const dx=Math.abs(event.clientX-cutX),dy=Math.abs(event.clientY-cutY); const progress=Math.min(1,Math.max(dx/85,dy/70)); if(dx>10||dy>10)cutMoved=true; guide.style.transform=`scaleX(${progress})`; if(progress>.58){ cutX=null; cutY=null; openPack(); } }
function endCut() { if(cutMoved&&!cutting) openPack(); cutX=null; cutY=null; }
...
booster.addEventListener('touchstart', event => { const touch=event.touches[0]; beginCut({clientX:touch.clientX,clientY:touch.clientY,pointerId:0}); }, {passive:true});
booster.addEventListener('touchmove', event => { const touch=event.touches[0]; continueCut({clientX:touch.clientX,clientY:touch.clientY}); }, {passive:true});
booster.addEventListener('touchend', () => { if (!cutting && cutMoved) openPack(); endCut(); }, { passive: true });
